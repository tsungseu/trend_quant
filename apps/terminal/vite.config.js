...
export default defineConfig(({ mode }) => {
  ...
  return {
    // 本地开发 base='/'；生产 web 占据站点根，终端迁至 /trend_quant/terminal/ 子路径，
    // 可用 VITE_PUBLIC_BASE 覆盖（注意：web 与终端由同一 Pages 站点托管，根路径不能重叠）
    base: env.VITE_PUBLIC_BASE || (process.env.NODE_ENV === 'production' ? '/trend_quant/terminal/' : '/'),
    plugins: [vue()],
    resolve: {
      alias: {
